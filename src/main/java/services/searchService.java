	...
	public ArrayList<Vehicle> getAllVehicles() {
		try {
			...
			while (rs.next()) {
				...
				String imagePth = vehicleService.getVehicleImagePath(rs.getInt("vehicleId"));
	...
	public ArrayList<Vehicle> searchAllVehicles(String title, int price_min, int price_max, int condition_nr,
		...
		try {
			...
			if (condition_nr != -1 || condition_nur != -1 || condition_ur != -1 || condition_uur != -1) {
				...
				parameters.add(condition_nr); // Add `null` if not applicable
				parameters.add(condition_nur);
				parameters.add(condition_ur);
				parameters.add(condition_uur);
			}
			...
			query += " ORDER BY vehicle.vehicleId DESC";
			...
			System.out.println("Prepared Query: " + pst);
